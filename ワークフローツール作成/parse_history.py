...
catalog_path = os.path.join(workspace_dir, "社内システム開発・運用ツール総合カタログ.md")
...
def main():
    ...
    # 2. 全チャットIDのリスト取得と英語タイトルのスキャン
    try:
        chat_ids = [d for d in os.listdir(brain_dir) if os.path.isdir(os.path.join(brain_dir, d)) and not d.startswith('.')]
