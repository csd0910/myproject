...
if ($startIndex -ge 0 -and $endIndex -gt $startIndex) {
    ...
    $newLines += 'Write-Host "`n[Network Configuration]" -ForegroundColor Cyan'
    $newLines += 'if ($setStaticIP -match "^y" -and -not [string]::IsNullOrWhiteSpace($ipAddress)) {'
    $newLines += '    Write-Host "Configuring Wired IP..."'
    ...
    $newLines += '        }'
    $newLines += '        Add-Check "Wired IP ($ipAddress)" "OK"'
    $newLines += '    } else {'
    $newLines += '        Add-Check "Wired IP" "NG (Adapter not found)"'
    $newLines += '    }'
    $newLines += '}'
    $newLines += ''
    $newLines += '# 2. Wi-Fi Configuration'
    $newLines += 'if ($setWifi -match "^y" -and -not [string]::IsNullOrWhiteSpace($wifiSSID)) {'
    $newLines += '    Write-Host "Configuring Wi-Fi Profile..."'
    ...
    $newLines += '    Add-Check "Wi-Fi ($wifiSSID)" "OK"'
    $newLines += '}'
    $newLines += ''
    $newLines += '# 3. NAS (Network Drive) Mapping'
    $newLines += 'Write-Host "Mounting NAS Y: Drive..."'
    ...
    $newLines += '    Write-Host "Trying fallback IP..." -ForegroundColor Yellow'
    ...
    $newLines += '    Add-Check "NAS Y:" "OK"'
    $newLines += '} else {'
    $newLines += '    Add-Check "NAS Y:" "NG"'
    $newLines += '}'
    $newLines += ''
