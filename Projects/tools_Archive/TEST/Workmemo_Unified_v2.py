...
class SettingsWindow(tk.Toplevel):
    ...
    def setup_ui(self):
        ...
        ttk.Label(main_frame, text="担当者名:", font=(APP_FONT, 9, "bold")).pack(anchor=tk.W)
        ...
        ttk.Label(main_frame, text="呼出ホットキー (例: ctrl+shift+w):", font=(APP_FONT, 9, "bold")).pack(anchor=tk.W)
        ...
        ttk.Label(main_frame, text="状況リスト (カンマ区切り):", font=(APP_FONT, 9, "bold")).pack(anchor=tk.W)
        self.txt_status = tk.Text(main_frame, height=3, font=(APP_FONT, 9))
        ...
        ttk.Label(s_frame, text="PC起動時に自動で常駐:", font=(APP_FONT, 8)).pack(side=tk.LEFT)
        ...
        ttk.Label(main_frame, text="※部署マスタの変更は config_unified.json を編集してください。", foreground="gray", font=(APP_FONT, 8)).pack(anchor=tk.W)
...
class UnifiedApp:
    def __init__(self, root):
        ...
        # フォントの明示的指定 (他PCでの明朝体化を防ぐ)
        self.font_name = APP_FONT
        self.default_font = (self.font_name, 9)
        
        # 標準Tkウィジェットのデフォルトフォント設定
        self.root.option_add("*Font", f"{{{self.font_name}}} 9")
    ...
    def apply_style(self):
        ...
        self.style.configure(".", font=self.default_font) # 全体
        self.style.configure("TFrame", background="#f0f2f5")
        self.style.configure("TLabel", background="#f0f2f5")
        self.style.configure("Header.TLabel", font=(self.font_name, 9, "bold"))
        self.style.configure("Accent.TButton", font=(self.font_name, 9, "bold"), foreground="white", background="#007bff")
        # Treeview
        self.style.configure("Treeview", font=self.default_font, rowheight=25)
        self.style.configure("Treeview.Heading", font=(self.font_name, 9, "bold"))
    ...
    def build_mode_2(self):
        ...
        for label, key, h in text_fields:
            ...
            txt = tk.Text(tf, height=h, font=self.default_font, bd=1)
...
if __name__ == "__main__":
    ...
    mutex_name = "WorkmemoUnified_v2_Mutex"
    ...
    if kernel32.GetLastError() == 183: # ERROR_ALREADY_EXISTS
        ...
        root.withdraw()
        # ここでもフォントを明示
        root.option_add("*Font", f"{{{APP_FONT}}} 9")
