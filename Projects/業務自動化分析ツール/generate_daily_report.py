...
def generate_reports(target_dir=None):
    ...
    for f in os.listdir(target_dir):
        ...
        elif f.startswith("system_log_") and f.endswith(".csv") and "ai_evaluated" not in f:
...
<body class="p-6 md:p-12">
    ...
    if system_log:
        with open(system_log, "r", encoding="utf-8-sig") as f:
            raw_system_log = f.read()
            
        prompt_csv = f"""
以下のシステム操作ログ（カンマ区切りのCSV形式）は、ユーザーがPC（主にExcel）で操作した生の記録です。
このログを元に、各行の右端（6列目）に新しく「AI解析要約」という列を追加して、人間らしい操作の解説を追記したCSVを出力してください。

【厳守事項】
- 出力は必ずプレーンなCSV形式テキストのみ（```csvなどのMarkdown記号は一切不要）とし、ヘッダーは (Timestamp, ActiveApp, EventType, TargetName, Metadata, AI解析要約) としてください。
...
"""
        try:
            print("[AI] システムログの全行解析（CSV一括生成）を実行中...")
            ...
            file_csv = os.path.join(target_dir, f"system_log_ai_evaluated_{target_date}.csv")
            with open(file_csv, "w", encoding="utf-8-sig") as f:
                f.write(csv_text)
            print(f"[完了] AI要約付きCSV生成: {file_csv}")
        except Exception as e:
            print(f"[エラー] AI要約付きCSV: {e}")
