...
def clean_label_perfect_v7(content):
    res = content
    # 1. ゴミ掃除
    ...
    # 2. 業務名への置換
    low = res.lower()
    if 'admin.microsoft' in low or 'admin.cloud' in low or 'microsoft 365 管理センター' in low:
        return 'Microsoft 365 管理センター'
    if 'amazon.co.jp' in low or 'amazonでの周辺機器' in res:
        return 'Amazonでの周辺機器・PC選定'
    if 'freee' in low:
        return 'freee人事労務（勤怠・申請）'
    if 'eset' in low:
        if 'import' in low or 'マスタ' in res: return 'ESET Webブロックマスタの管理・配布'
        return 'ESET Endpoint Securityの設定・確認'
    if 'bing.com' in low or 'google.com' in low or 'google 検索' in res or 'google.co.jp' in low:
    ...
    # 3. 未置換の長いURL（ゴミ）をキャッチ
    if 'http' in low or '/' in res or '?' in res or '.com' in low:
        return '関連サイト・ウェブ資料での調査'
...
def format_helpdesk_final_perfect():
    input_file = r"C:\Users\フォーレスト026\MyProject\tools\Memo\ActivityLog_0410_0512.csv"
    output_file = r"C:\Users\フォーレスト026\MyProject\tools\Memo\Helpdesk_Report_Final_v7.txt"
    ...
    with open(input_file, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            if is_helpdesk_task(clean_c):
                label = clean_label_perfect_v7(clean_c)
    ...
    print(f"Final perfect report created at: {output_file}")

if __name__ == "__main__":
    format_helpdesk_final_perfect()
