...
def get_browser_history(browser_name, history_path):
    history_items = []
    temp_history = f"scratch/{browser_name}_final_history_v3"
...
def get_windows_activity():
    ...
    for root, dirs, files in os.walk(base_path):
        if 'ActivitiesCache.db' in files:
            ...
            temp_db = f"scratch/activity_cache_v3_{os.path.basename(root)}"
            try:
...
def get_external_log(csv_path):
    ...
    # Column G (index 6), Column P (index 15)
    try:
        ...
        for enc in ['utf-8-sig', 'cp932', 'utf-8']:
            try:
                with open(csv_path, 'r', encoding=enc) as f:
                    ...
                    for row in reader:
                        if len(row) > 15:
                            time_str = row[6] # Column G
                            detail = row[15] # Column P
                            try:
                                # Example: 2026/05/12 15:09:49
