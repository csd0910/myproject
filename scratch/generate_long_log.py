...
def get_browser_history(browser_name, history_path):
    history_items = []
    temp_history = f"scratch/{browser_name}_long_history_temp"
    if os.path.exists(history_path):
        try:
            ...
            for row in cursor.fetchall():
                ...
                if dt <= END_DATE:
                    history_items.append({'time': dt, 'type': 'ブラウザ', 'detail': f"閲覧: {row[2][:50]}"})
            conn.close()
        except: pass
...
def get_windows_activity():
    ...
    for root, dirs, files in os.walk(base_path):
        if 'ActivitiesCache.db' in files:
            ...
            temp_db = "scratch/activity_cache_temp"
            try:
                shutil.copy2(db_path, temp_db)
                conn = sqlite3.connect(temp_db)
                cursor = conn.cursor()
                # Windows Activity Time is Unix epoch
                start_ts = int(START_DATE.timestamp())
                # Activity type 5 is often app usage
                ...
                for row in cursor.fetchall():
                    ...
                    if dt <= END_DATE:
                        ...
                        try:
                            ...
                            app_name = app_info[0]['packageId'] if isinstance(app_info, list) else row[1]
                        except:
                            app_name = row[1]
                        activity_items.append({'time': dt, 'type': 'アプリ', 'detail': f"利用: {app_name} ({row[2] or ''})"})
                conn.close()
            except: pass
        ...
        for root, dirs, files in os.walk(brain_dir):
            if 'overview.txt' in files:
                ...
                try:
                    ...
                    if dt >= START_DATE:
                        # Antigravity logs don't have per-message timestamps in overview.txt easily parsable here
                        # but we can at least log the conversation update
                        logs.append({'time': dt, 'type': 'AI相談', 'detail': f"AI対話履歴の更新 ({root.split(os.sep)[-2]})"})
                except: pass
    return logs
