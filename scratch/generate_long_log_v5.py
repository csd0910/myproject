...
def get_browser_history(browser_name, history_path):
    ...
    temp_history = f"scratch/{browser_name}_{os.path.basename(os.path.dirname(history_path))}_temp"
    try:
        ...
        for row in cursor.fetchall():
            ...
            if dt <= END_DATE:
                ...
                history_items.append({'time': dt, 'type': f'ブラウザ({browser_name})', 'detail': f"閲覧: {title}"})
        conn.close()
    except: pass
    return history_items

def get_downloaded_history(csv_path):
    history_items = []
            ...
            for row in reader:
                try:
                    ...
                    if START_DATE <= dt <= END_DATE:
                        history_items.append({'time': dt, 'type': 'ブラウザ(手動)', 'detail': f"閲覧: {row['title']}"})
                except: continue
    except: pass
    return history_items
...
def get_windows_activity():
    ...
    for root, dirs, files in os.walk(base_path):
        if 'ActivitiesCache.db' in files:
            ...
            temp_db = f"scratch/activity_cache_v5_{os.path.basename(root)}"
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    if dt <= END_DATE:
                        ...
                        if "Microsoft.Language.InputHistory" in app_name: continue
        ...
        for enc in ['utf-8-sig', 'cp932', 'utf-8']:
            try:
                with open(csv_path, 'r', encoding=enc) as f:
                    ...
                    for row in reader:
                        if len(row) > 15:
                            time_str, detail = row[6], row[15]
                            try:
                                dt = datetime.datetime.strptime(time_str, '%Y/%m/%d %H:%M:%S')
                                if START_DATE <= dt <= END_DATE:
                                    external_items.append({'time': dt, 'type': '外部ログ', 'detail': f"{detail}"})
                            except: continue
                break
            except: continue
    except: pass
...
def main():
    ...
    for profile in ['Default', 'Profile 1', 'Profile 2']:
        all_events.extend(get_browser_history("Edge", os.path.join(edge_base, profile, "History")))
    
    # Chrome Profiles
    chrome_base = os.path.expandvars(r"%LOCALAPPDATA%\Google\Chrome\User Data")
    for profile in ['Default', 'Profile 1']:
        all_events.extend(get_browser_history("Chrome", os.path.join(chrome_base, profile, "History")))

    all_events.extend(get_downloaded_history(r"C:\Users\フォーレスト026\Downloads\history (1).csv"))
    all_events.extend(get_windows_activity())
    all_events.extend(get_ai_logs())
    all_events.extend(get_external_log(r"C:\Users\フォーレスト026\Desktop\伊藤作業用\Cログ検索結果-20260512150949.csv"))
