...
# =========================================================
# STEP-1: 管理部提出用一覧表の自動作成 (VBA代替)
# =========================================================
def step1_create_submission_list(log_path, master_path, output_dir):
    try:
        print("台帳を読み込んでいます...")
        ...
        messagebox.showinfo("STEP-1 完了", f"【管理部提出用シート】を作成しました！\nまずはこのファイルを管理部へ提出してください。\n\n出力先: {out_filepath}")
...
def step2_generate_reports(excel_path, csv_paths, output_dir):
    ...
    for csv_path in csv_paths:
        ...
        # VBA版のリストには打刻等の項目がないため、PC電源ON/OFFをベースに判定
...
class OvertimeSystemApp_V2:
    def __init__(self, root):
        ...
        btn1 = tk.Button(root, text="STEP-1: 【管理部提出用シート】の作成 (VBA代行)", command=self.run_step1, width=55, height=2, bg="#e0f7fa", font=("Meiryo", 10))
        btn1.pack(pady=10)
    ...
    def run_step1(self):
        messagebox.showinfo("STEP-1", "【1/3】SKYSEAから抽出した「生ログ(CSV)」を選択してください。")
