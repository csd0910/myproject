...
def get_japanese_status(smtp_code: str, diag_text: str) -> str:
    ...
    if re.search(r"5\.[01]\.[01]|user unknown|no such user|does not exist|mailbox unavailable", combined):
        return "【不在】宛先アドレスが存在しません"
    if re.search(r"5\.2\.2|over.?quota|mailbox full|storage|quota.?exceed|552", combined):
        return "【満杯】相手の容量不足です"
    if re.search(r"5\.7\.[0-9]|blocked|spam|reject|policy|dmarc|spf.?fail|dkim.?fail|ban", combined):
        return "【拒否】拒否設定または認証不備による拒否です"
    if re.search(r"(?<!\d)4\.\d\.\d|421|450|451|452", combined):
        return "【一時的】再送対象エラーです"
    if re.search(r"(?<!\d)5\.\d\.\d|5[0-9][0-9]", combined):
        return "【恒久的】配信不能エラーです"
    return "【不明】詳細確認が必要です"
